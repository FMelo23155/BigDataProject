#!/usr/bin/env python3
"""
Script para fazer merge das colunas Arousal e Valence do ficheiro merge_bimodal_complete_av_values.csv
para o ficheiro merge_bimodal.csv baseado no Song_id.
"""
...
def main():
    """Função principal para executar o merge dos dados bimodais."""
    
    # Caminhos dos ficheiros
    base_dir = Path(__file__).parent.parent
    target_file = base_dir / "metadata" / "base" / "merge_bimodal.csv"
    source_file = base_dir / "metadata" / "last" / "lastbase" / "merge_bimodal_complete_av_values.csv"
    
    print("=== Script de Merge - Bimodal Arousal e Valence ===")
    ...
    try:
        ...
        if 'Audio_Song' not in source_df.columns:
            print("ERRO: Coluna 'Audio_Song' não encontrada no ficheiro de origem")
            sys.exit(1)
        ...
        # Renomear a coluna 'Audio_Song' para 'Song_id' no dataframe de origem para facilitar o merge
        source_df_renamed = source_df.rename(columns={'Audio_Song': 'Song_id'})
        ...
        sample_data = merged_df[['Song_id', 'Lyric_Song', 'Artist', 'Title', 'Arousal', 'Valence']].head(3)
        for _, row in sample_data.iterrows():
            print(f"   - {row['Song_id']} ({row['Lyric_Song']}): {row['Artist']} - {row['Title']} | Arousal: {row['Arousal']}, Valence: {row['Valence']}")
