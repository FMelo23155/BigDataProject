#!/usr/bin/env python3
"""
Script para validar se os Song IDs do ficheiro merge_bimodal.csv estão presentes no ficheiro 
merge_bimodal_balanced_metadata.csv e adicionar uma coluna 'in_bimodal_balanced' com True/False.
"""
...
def main():
    ...
    target_file = base_dir / "metadata" / "base" / "merge_bimodal.csv"
    reference_file = base_dir / "metadata" / "last" / "lastbase" / "merge_bimodal_balanced_metadata.csv"
    
    print("=== Script de Validação - Bimodal Balanced ===")
    ...
    try:
        ...
        if 'Audio_Song' not in reference_df.columns:
            print("ERRO: Coluna 'Audio_Song' não encontrada no ficheiro de referência")
            sys.exit(1)
        
        # Verificar se a coluna in_bimodal_balanced já existe
        if 'in_bimodal_balanced' in target_df.columns:
            print("AVISO: Coluna 'in_bimodal_balanced' já existe no ficheiro principal")
        ...
        print("\n2. A processar dados...")
        reference_songs = set(reference_df['Audio_Song'].values)
        
        # Criar a coluna in_bimodal_balanced
        target_df['in_bimodal_balanced'] = target_df['Song_id'].isin(reference_songs)
        
        # Validações e estatísticas
        print("\n3. A validar resultados...")
        
        total_songs = len(target_df)
        songs_in_balanced = target_df['in_bimodal_balanced'].sum()
        ...
        songs_present = target_df[target_df['in_bimodal_balanced'] == True][['Song_id', 'Artist', 'Title']].head(3)
        ...
        songs_absent = target_df[target_df['in_bimodal_balanced'] == False][['Song_id', 'Artist', 'Title']].head(3)
        ...
        print(f"   - Coluna 'in_bimodal_balanced' adicionada")
        ...
        if 'Quadrant' in target_df.columns:
            ...
            quadrant_stats = target_df.groupby('Quadrant')['in_bimodal_balanced'].agg(['count', 'sum']).reset_index()
