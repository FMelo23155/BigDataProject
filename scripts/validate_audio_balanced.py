#!/usr/bin/env python3
"""
Script para validar se os Song IDs do ficheiro merge_audio.csv estão presentes no ficheiro 
merge_audio_balanced_metadata.csv e adicionar uma coluna 'in_audio_balanced' com True/False.
"""
...
def main():
    ...
    target_file = base_dir / "metadata" / "base" / "merge_audio.csv"
    reference_file = base_dir / "metadata" / "last" / "lastbase" / "merge_audio_balanced_metadata.csv"
    
    print("=== Script de Validação - Audio Balanced ===")
    ...
    try:
        ...
        if 'Song' not in target_df.columns:
            print("ERRO: Coluna 'Song' não encontrada no ficheiro principal")
            sys.exit(1)
        ...
        # Verificar se a coluna in_audio_balanced já existe
        if 'in_audio_balanced' in target_df.columns:
            print("AVISO: Coluna 'in_audio_balanced' já existe no ficheiro principal")
        ...
        # Criar a coluna in_audio_balanced
        target_df['in_audio_balanced'] = target_df['Song'].isin(reference_songs)
        
        # Validações e estatísticas
        print("\n3. A validar resultados...")
        
        total_songs = len(target_df)
        songs_in_balanced = target_df['in_audio_balanced'].sum()
        ...
        songs_present = target_df[target_df['in_audio_balanced'] == True][['Song', 'Artist', 'Title']].head(3)
        if len(songs_present) > 0:
            print("   Songs presentes no balanced:")
            for _, row in songs_present.iterrows():
                print(f"     ✓ {row['Song']}: {row['Artist']} - {row['Title']}")
        
        # Songs que não estão no balanced
        songs_absent = target_df[target_df['in_audio_balanced'] == False][['Song', 'Artist', 'Title']].head(3)
        ...
        target_songs = set(target_df['Song'].values)
        ...
        print(f"   - Coluna 'in_audio_balanced' adicionada")
        ...
        if 'Quadrant' in target_df.columns:
            ...
            quadrant_stats = target_df.groupby('Quadrant')['in_audio_balanced'].agg(['count', 'sum']).reset_index()
